...
if(!class_exists('meris_breadcrumb'))
{

	class meris_breadcrumb
	{
		var $options;
		
	function meris_breadcrumb($options = ""){
		...
		global $post;
		echo '<div class="meris-breadcrumb"><a href="'.esc_url(home_url("/")).'">';
			_e("Home",'meris');
			echo "</a>";
			if(!is_front_page()){echo $markup;}
					
			$output = $this->meris_simple_breadcrumb_case($post);
		...
		function meris_simple_breadcrumb_case($der_post)
		{
			global $post; 
			...
			if (is_page()){
				 if($der_post->post_parent) {
					 ...
					 $this->meris_simple_breadcrumb_case($my_query);
					 $link = '<a href="';
		...
		if(is_author()){
			...
			return __("Author",'meris').": ".$curauth->nickname;
		}
		if(is_tag()){ return __("Tag",'meris').": ".single_tag_title('',FALSE); }
		
		if(is_404()){ return _e("404 - Page not Found",'meris'); }
		
		if(is_search()){ return _e("Search",'meris');}	
...
/*-----------------------------------------------------------------------------------*/
/* meris_breadcrumbs() - Custom breadcrumb generator function  */
/*
/* Params:
/*
/* Arguments Array:
...
function meris_breadcrumbs( $args = array() ) {
	...
	$defaults = array(
		'separator' => '&raquo;',
		'before' => '<span class="breadcrumb-title">' . __( 'You are here:', 'meris') . '</span>',
		'after' => false,
		'front_page' => true,
		'show_home' => __( 'Home', 'meris'),
		'echo' => false, 
	...
	$args = apply_filters( 'meris_breadcrumbs_args', $args );
	...
	elseif ( is_home() ) {
		...
		$trail = array_merge( $trail, meris_breadcrumbs_get_parents( $home_page->post_parent, '' ) );
	...
	elseif ( is_singular() ) {
		...
		if ( 'page' !== $post_type && 'post' !== $post_type ) {
			...
			if ( !empty( $path ) )
				$trail = array_merge( $trail, meris_breadcrumbs_get_parents( '', $path ) );
		...
		*/
		...
		if ( empty( $path ) && 0 !== $parent || 'attachment' == $post_type )
			$trail = array_merge( $trail, meris_breadcrumbs_get_parents( $parent, '' ) );
		...
		if ( 'post' == $post_type && $show_posts_page == true && 'page' == get_option( 'show_on_front' ) ) {
			...
			if ( $posts_page != '' && is_numeric( $posts_page ) ) {
				$trail = array_merge( $trail, meris_breadcrumbs_get_parents( $posts_page, '' ) );
			}
		}
	...
	elseif ( is_archive() ) {
		...
		if ( is_tax() || is_category() || is_tag() ) {
			...
			if ( is_taxonomy_hierarchical( $term->taxonomy ) && $term->parent )
				$trail = array_merge( $trail, meris_breadcrumbs_get_term_parents( $term->parent, $term->taxonomy ) );
		...
		elseif ( is_time() ) {
			...
			if ( get_query_var( 'minute' ) && get_query_var( 'hour' ) )
				$trail['trail_end'] = get_the_time( __( 'g:i a', 'meris') );

			elseif ( get_query_var( 'minute' ) )
				$trail['trail_end'] = sprintf( __( 'Minute %1$s', 'meris'), get_the_time( __( 'i', 'meris') ) );

			elseif ( get_query_var( 'hour' ) )
				$trail['trail_end'] = get_the_time( __( 'g a', 'meris') );
		}
		...
		elseif ( is_date() ) {
			...
			if ( $wp_rewrite->front )
				$trail = array_merge( $trail, meris_breadcrumbs_get_parents( '', $wp_rewrite->front ) );

			if ( is_day() ) {
				$trail[] = '<a href="' . get_year_link( get_the_time( 'Y' ) ) . '" title="' . get_the_time( esc_attr__( 'Y', 'meris') ) . '">' . get_the_time( __( 'Y', 'meris') ) . '</a>';
				$trail[] = '<a href="' . get_month_link( get_the_time( 'Y' ), get_the_time( 'm' ) ) . '" title="' . get_the_time( esc_attr__( 'F', 'meris') ) . '">' . get_the_time( __( 'F', 'meris') ) . '</a>';
				$trail['trail_end'] = get_the_time( __( 'j', 'meris') );
			}

			elseif ( get_query_var( 'w' ) ) {
				$trail[] = '<a href="' . get_year_link( get_the_time( 'Y' ) ) . '" title="' . get_the_time( esc_attr__( 'Y', 'meris') ) . '">' . get_the_time( __( 'Y', 'meris') ) . '</a>';
				$trail['trail_end'] = sprintf( __( 'Week %1$s', 'meris'), get_the_time( esc_attr__( 'W', 'meris') ) );
			}

			elseif ( is_month() ) {
				$trail[] = '<a href="' . get_year_link( get_the_time( 'Y' ) ) . '" title="' . get_the_time( esc_attr__( 'Y', 'meris') ) . '">' . get_the_time( __( 'Y', 'meris') ) . '</a>';
				$trail['trail_end'] = get_the_time( __( 'F', 'meris') );
			}

			elseif ( is_year() ) {
				$trail['trail_end'] = get_the_time( __( 'Y', 'meris') );
			}
		}
	}
	...
	elseif ( is_search() )
		$trail['trail_end'] = sprintf( __( 'Search results for &quot;%1$s&quot;', 'meris'), esc_attr( get_search_query() ) );

	/* If viewing a 404 error page. */
	elseif ( is_404() )
		$trail['trail_end'] = __( '404 Not Found', 'meris');

	/* Allow child themes/plugins to filter the trail array. */
	$trail = apply_filters( 'meris_breadcrumbs_trail', $trail, $args );
	...
	$breadcrumb = apply_filters( 'meris_breadcrumbs', $breadcrumb );
...
} // End meris_breadcrumbs()


function meris_breadcrumbs_get_parents( $post_id = '', $path = '' ) {
...
} // End meris_breadcrumbs_get_parents()


function meris_breadcrumbs_get_term_parents( $parent_id = '', $taxonomy = '' ) {
...
} // End meris_breadcrumbs_get_term_parents()