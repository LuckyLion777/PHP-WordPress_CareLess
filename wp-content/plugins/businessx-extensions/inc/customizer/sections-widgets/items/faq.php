...
if( ! class_exists( 'Businessx_Extensions_Faq_Item' ) ) {
	class Businessx_Extensions_Faq_Item extends Businessx_Extensions_Base {
		...
		function __construct() {
			...
			// Variables
			$this->widget_title = __( 'BX: Question' , 'businessx-extensions' );
			$this->widget_id = 'faq';
		...
		public function widget( $args, $instance ) {
			...
			$allowed_html = apply_filters( 'businessx_extensions_faq_item___allowed_html', $allowed_html = $this->allowed_html ); set_query_var( 'allowed_html', $allowed_html ); 

			// Add more widget classes
			$css_class = array();
			$css_class[] = 'grid-col';
			$css_class[] = 'grid-2x-col';
			$css_class[] = 'sec-faq-question';
			$css_class = apply_filters( 'businessx_extensions_faq_item___css_classes', $css_class );
			...
			echo $args['before_widget'];
				...
				businessx_extensions_get_template_part( 'sections-items/item', 'faq' );
		...
		public function update( $new_instance, $old_instance ) {
			...
			// Variables
			$allowed_html = apply_filters( 'businessx_extensions_faq_item___allowed_html', $allowed_html = $this->allowed_html );
		...
		public function form( $instance ) {
			...
			/* Excerpt */
			parent::text_area( $excerpt, 'excerpt', __( 'Answear:', 'businessx-extensions' ), '', '', __( 'Allowed html tags: <a>, <strong>, <em>.', 'businessx-extensions' ) );
		}
		
		
	} // Businessx_Extensions_Faq_Item .END
	
	// Register this widget
	register_widget( 'Businessx_Extensions_Faq_Item' );
